...
export default function Navbar() {
  ...
  const [showMobileBatchMenu, setShowMobileBatchMenu] = useState(false);

  const batchDropdownRef = useRef(null);
  const mobileBatchDropdownRef = useRef(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (mobileBatchDropdownRef.current && !mobileBatchDropdownRef.current.contains(event.target)) {
        setShowMobileBatchMenu(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 space-y-2 sm:space-y-0">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              {/* Batch / Regulation Year Dropdown */}
              <div className="relative border-l border-slate-300 pl-3 ml-1" ref={batchDropdownRef}>
                <button
                  type="button"
                  id="batch-year-dropdown-btn"
                  onClick={() => setShowBatchMenu(!showBatchMenu)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100/90 hover:bg-slate-200/90 border border-slate-200/90 text-xs font-black text-slate-700 hover:text-indigo-600 transition-all shadow-xs cursor-pointer select-none"
                  title="Select Regulation / Batch Year"
                >
                  ...
                  <ChevronDown
                    className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${
                      showBatchMenu ? 'rotate-180 text-indigo-600' : ''
                    }`}
                  />
                </button>

                {showBatchMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-white py-1.5 shadow-2xl rounded-2xl border border-slate-200 z-50 animate-fade-in text-xs font-bold">
                    <div className="px-3.5 py-1.5 text-[10px] font-black uppercase tracking-wider text-slate-400 border-b border-slate-100">
                      Grading Batch
                    </div>
                    <button
                      type="button"
                      id="select-before-2026-btn"
                      onClick={() => {
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-medium text-slate-400">Legacy E-grade (Sem 1 & 2)</span>
                      </div>
                      {batchYear === 'before-2026' && (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-medium text-slate-400">Standard P-grade (Sem 1 & 2)</span>
                      </div>
                      {batchYear === 'after-2026' && (
                        <CheckCircle2 className="w-4 h-4 text-indigo-600 shrink-0 ml-1" />
                      )}
          ...
          <div className="flex md:hidden items-center justify-between gap-2 pt-1 border-t border-slate-100">
            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-1 w-full text-[11px] font-bold">
              ...
              {/* Batch Dropdown on Mobile */}
              <div className={`relative shrink-0 ${viewMode !== 'semester' ? 'ml-auto' : ''}`} ref={mobileBatchDropdownRef}>
                <button
                  type="button"
                  id="mobile-batch-year-dropdown-btn"
                  onClick={() => setShowMobileBatchMenu(!showMobileBatchMenu)}
                  className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-200 text-[10px] font-extrabold text-slate-700"
                >
                  <Calendar className="w-3 h-3 text-indigo-600" />
                  <span>{batchYear === 'after-2026' ? 'After 2026' : 'Before 2026'}</span>
                  <ChevronDown
                    className={`w-3 h-3 text-slate-400 transition-transform ${
                      showMobileBatchMenu ? 'rotate-180 text-indigo-600' : ''
                    }`}
                  />
                </button>

                {showMobileBatchMenu && (
                  <div className="absolute right-0 mt-1.5 w-44 bg-white py-1 shadow-2xl rounded-xl border border-slate-200 z-50 text-[11px] font-bold">
                    <button
                      type="button"
                      id="mobile-select-before-2026-btn"
                      onClick={() => {
                        setBatchYear('before-2026');
                        setShowMobileBatchMenu(false);
                      }}
                    ...
                    >
                      <div>
                        <div>Before 2026</div>
                        <div className="text-[9px] font-normal text-slate-400">Legacy E-grade</div>
                      </div>
                      {batchYear === 'before-2026' && (
                    ...
                    <button
                      type="button"
                      id="mobile-select-after-2026-btn"
                      onClick={() => {
                        setBatchYear('after-2026');
                        setShowMobileBatchMenu(false);
                      }}
                    ...
                    >
                      <div>
                        <div>After 2026</div>
                        <div className="text-[9px] font-normal text-slate-400">Standard P-grade</div>
                      </div>
                      {batchYear === 'after-2026' && (
                        <CheckCircle2 className="w-3.5 h-3.5 text-indigo-600" />
                      )}
